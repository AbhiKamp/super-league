...
export async function GET(request: NextRequest) {
  ...
  // The official 4 teams that qualified for Semi-Finals
  ...
  try {
    ...
    // 2. Fetch ungraded Semi-Final predictions
    // IMPORTANT: Make sure you created the 'sf_points_awarded' column and set default to 0!
    const { data: predictions, error: predErr } = await supabase
        .from('wc_knockout_predictions')
        .select('*')
        .eq('sf_points_awarded', 0);
        
    if (predErr) throw predErr;
    if (!predictions || predictions.length === 0) {
        return NextResponse.json({ status: "All users have already been graded for Semi-Finals!" });
    }

    const userScores: Record<string, number> = {};
    const updatesToMake: { id: string, sf_points_awarded: number, matches: number }[] = [];

    // 3. Grade the predictions
    predictions.forEach(pred => {
        // Parse the semi_finals array
        let predictedSF: any[] = [];
        try {
            predictedSF = typeof pred.semi_finals === 'string' ? JSON.parse(pred.semi_finals) : pred.semi_finals;
        } catch (e) {
            console.error(`Failed to parse semi_finals for user ${pred.user_id}`);
            return;
        }

        // Convert all predicted IDs to numbers for safe matching
        const predictedIds = predictedSF.map(Number);
        ...
        // 150 points per correct team
        const pointsEarned = correctMatches * 150;

        updatesToMake.push({ 
            id: pred.id, 
            sf_points_awarded: pointsEarned,
    ...
    if (execute) {
        ...
        for (let i = 0; i < updatesToMake.length; i += chunkSize) {
            ...
            await Promise.all(chunk.map(update => 
                supabase
                    .from('wc_knockout_predictions')
                    .update({ sf_points_awarded: update.sf_points_awarded })
                    .eq('id', update.id)
            ));
        }
        ...
        return NextResponse.json({ 
            status: "SUCCESS: Semi-Finals graded and leaderboard updated!", 
